...
To capture this historical information to guide the updating of the strategy by RL, we make the first term of Eq. \eqref{exem}:
\begin{equation}
...
Under the new constraints, the objective of optimizing $W_{\alpha}$ and $W_{\beta}$ is to find a set of optimal weights such that the combination of $ p_{i,d}$ and $g_{d}$ improves the global convergence of the PSO. We can define an optimization objective function to evaluate the performance under different weights:
\begin{equation}
...
With the help of RL, make $W_{\alpha}$ and $W_{\beta}$ dynamic tuning. In each iteration $t$, PPO is used to update the weights in response to the current optimization requirements:
\begin{equation}
...
$A_{l}$ denotes the action labels sampled from the distribution of the discrete action space. $AL$ is a finite set of real numbers $\mathbb{R}$:
\begin{equation}
	\label{al}
	AL = \{0, 1, 2, \dots\},
\end{equation}
...
\begin{table}[t]
	...
	\end{tabular}
	\end{adjustwidth}
	\caption{Sensitivity analysis of each hyper-parameter using variance decomposition (Sobol).}
...
\begin{figure}[t]
	...
	\caption{Convergence comparison for four wind conditions with and without RL strategies.}
